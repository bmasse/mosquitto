...
int main(int argc, char *argv[])
{
	...
	mosquitto_lib_init();  // Benoit init rand() et c'est tout.
	BLog();

	output_init();  // Benoit pas important.
	BLog();
	...
	g_mosq = mosquitto_new(cfg.id, cfg.clean_session, &cfg); // Benoit Alloue la struct mosquitto et la nettoie 
	if(!g_mosq){
		switch(errno){
			case ENOMEM:
	...
	if(client_opts_set(g_mosq, &cfg)){  // Benoit validation 
		goto cleanup;
	}
	if(cfg.debug){
	...
	rc = mosquitto_loop_forever(g_mosq, -1, 5);
