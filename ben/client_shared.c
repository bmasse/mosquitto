...
/*
Benoit valide les parametres d'entres 
Valide pour mosquitto_sub, _pub et _rr.
*/
...
fprintf(stderr, "0 Benoit Benoit\n\n");
printf("1 Benoit Benoit\n\n");
	for(i=1; i<argc; i++){
		if(!strcmp(argv[i], "-A")){
			if(i==argc-1){
...
/*
Benoit Initialise certains parametres du tls.
*/
...
int client_connect(struct mosquitto *mosq, struct mosq_config *cfg)
{
	...
	if(cfg->port == PORT_UNDEFINED){
		BLog("cfg->port is PORT_UNDEFINED ");
#ifdef WITH_TLS
		if(cfg->cafile || cfg->capath
#  ifdef FINAL_WITH_TLS_PSK
				|| cfg->psk
...
#ifdef WITH_SRV  // Benoit pas defini
BLog("WITH_SRV");
	if(cfg->use_srv){
